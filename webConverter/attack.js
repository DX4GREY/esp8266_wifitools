...
function draw() {
	...
	getE("startAttack").innerHTML = attackJSON[0][0] | attackJSON[1][0] | attackJSON[2][0] | attackJSON[3][0] | attackJSON[4][0] ? lang("stop") : lang("start");
...
function start() {
	...
	// 		break;
	// }
	if (attackJSON[0][0] | attackJSON[1][0] | attackJSON[2][0] | attackJSON[3][0] | attackJSON[4][0]){
		stopAll();
	}else{
		attackJSON[0][0] = getE("deauth").checked;
		attackJSON[1][0] = getE("beacon").checked;
		attackJSON[2][0] = getE("probe").checked;
		attackJSON[3][0] = getE("deauthAll").checked;
		attackJSON[4][0] = getE("evilTwin").checked;
...
function load() {
	getFile("attack.json", function (response) {
		...
		getE("deauth").checked = attackJSON[0][0];
		getE("beacon").checked = attackJSON[1][0];
		getE("probe").checked = attackJSON[2][0];
		getE("deauthAll").checked = attackJSON[3][0];
		getE("evilTwin").checked = attackJSON[4][0];
