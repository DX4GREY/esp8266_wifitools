...
function minify_html_css {
    file=$1
    curl -X POST -s --data-urlencode "input@$file" https://www.toptal.com/developers/html-minifier/api/raw > /tmp/converter.temp
}

function minify_js {
    file=$1
    curl -X POST -s --data-urlencode "input@$file" https://www.toptal.com/developers/javascript-minifier/api/raw > /tmp/converter.temp
}
